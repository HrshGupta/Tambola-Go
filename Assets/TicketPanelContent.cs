...
public class TicketPanelContent : MonoBehaviour
{
    public Color[] colors;
    ...
    void Update()
    {
        ...
        for (int i = 0; i < pos.Length; i++)
        {
            if (scroll_pos < pos[i] + (distance / 2) && scroll_pos > pos[i] - (distance / 2))
            {
                ...
                imageContent.transform.GetChild(i).localScale = Vector2.Lerp(imageContent.transform.GetChild(i).localScale, new Vector2(2f, 2f), 0.1f);
